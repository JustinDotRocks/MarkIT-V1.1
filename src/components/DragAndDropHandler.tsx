...
const DragAndDropHandler: React.FC<DragAndDropHandlerProps> = ({
  ...
  const handleDragMove = (e: any) => {
    ...
    //! Use of includes for type checking might lead to incorrect type checks if new types are added in the future. You could use a switch statement instead.
    ...
    //! This method of getting the width and height isn't very scalable if you add more item types. You could create a configuartion object that maps item types to their dimensions.
    /* ex: 
		const TABLE_DIMENSIONS = { 
			"table-5": { width: 5, height: 2.5 },
		    "table-6": { width: 6, height: 2.5 }, 
			"table-8": { width: 8, height: 2.5 }
		} */
    const itemWidthFeet = type === "table-6" ? 6 : type === "table-8" ? 8 : 5;
    const itemHeightFeet = type.includes("table") ? 2.5 : 1.5;

    //! Doesn't seem to be a need to reassign these values here ?
