...
//! Repeated string literal types can be extracted into a union type that can be used across multiple types/interfaces
/*
...
//! is the Generic required here ? featureType can probably just be the union type
...
export interface RoomSetupModalProps {
  ...
  editRoom?: (id: string, name: string, width: string, depth: string) => void;
...
//! properties that are used across multiple components can be extracted into a type alias. NOTE: this doesn't mean for properties that just have a common name, but for properties that have the same type and are used in the same way. ex: export type RoomId = string | null
...
//! Table seems to use all of the same properties as Feature, so you could have Table extend Feature and then add the additional properties
...
//! You can use ? to make the property optional instead of using | undefined
//! There should usually be a type for event handlers, like React.MouseEvent<HTMLButtonElement> -- if it's a Konva component, check to see if Konva has a type system
