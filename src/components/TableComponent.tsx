...
const TableComponent: React.FC<DragAndDropComponentProps> = ({
  ...
  //! there is a note on doing this elsewhere, this is something that can be exported into constants and imported where needed
  ...
  const scaleFactor =
    (dominantRoomDimension === "width"
      ? containerSize.width / (parseFloat(room?.width || "1") * feetToPixels)
      : containerSize.width / (parseFloat(room?.depth || "1") * feetToPixels)) *
    scalingAdjustment;
  ...
  //! a lot of "Magic Numbers" here -- add some comments to explain what these numbers represent
  //! another though -- is there a clean way to calculate these values in a function to create a sliding scale based on the room width?
  ...
  //! can extract colors into a constant object ex: const COLORS = { VENDOR_GREEN: "#496f3d", VENDOR_RED: "#de5e46", VENDOR_GREY: "#628b98" }
  ...
  return (
    ...
    <React.Fragment>
      {isCircle ? (
        //! a lot of common props between the Circle and Rect components, could be extracted into a common props object and then spread into each one to reduce duplication
        /*
		const commonProps = {
			id: table.id,
			...
			onDragEnd,
			onClick: (e) =>
				onObjectClick(table.id, "table", e.evt.clientX, e.evt.clientY),
